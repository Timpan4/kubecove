...
- Connected Argo CD uses an eligible HTTPS profile for bounded, whole-result inspection and reviewed operations. When a Connected inspection fails, the backend discards partial Connected data and, when possible, returns one complete Kubernetes result with visible fallback provenance.
...
Argo operation reviews are five-minute, single-use Rust sessions. Durable session state uses only native credential storage. Before execution, Rust revalidates the allowlisted request, exact scope, available connection or Kubernetes client, and Application identity; resource actions are checked against the current server report. Execution remains bound to the reviewed transport.

Resource Browser and GitOps provide `Refresh (clear cache)` inside a compact live-status menu, in the resource status area and beside the GitOps heading. It clears the relevant backend cache and reloads active reads, including open details, without resetting filters or selection. The menu shows connection details and refresh failures without a dedicated refresh row. Each resource watch reports its own health. While any watch is disconnected, KubeCove retries startup and reloads the view at the existing 15-second Argo inspection cadence; established watches reconnect in the backend. Normal watch renewals preserve connection status and do not trigger a full refresh. New views receive the current state of shared watches immediately. Reload failures remain distinct from connection failures and retry until they succeed.

Argo CD Refresh and Hard refresh remain separate controller operations. Both stay disabled while an operation is pending. KubeCove reads the current Application before review and tracks accepted operations until controller evidence confirms completion or failure. An observation failure remains visible and retries without resubmitting the operation. Tracking continues across views within the same workspace and cluster context while KubeCove remains open. Application lists follow watch events, with periodic reloads only during recovery, and show observed operation phases and refresh requests, including external operations; details show resource results and failure messages supplied by Argo CD.
...
Priority evidence surfaces include metadata, owner references, conditions, events, logs, metrics when available, deployment revisions, topology, normalized Argo CD and Flux ownership evidence, GitOps status, Helm reconciliation, and RBAC provenance. Resource Browser, Resource Detail, topology, and Incident Workbench consume the same GitOps ownership presentation rules.
...
Connected inspection may visibly replace a failed whole-result attempt with one complete Kubernetes result; it never mixes transports. Argo operation sessions bind execution to the reviewed transport and never switch after confirmation. RBAC inventory is observed policy evidence; exact authorization uses an explicit live access review.
