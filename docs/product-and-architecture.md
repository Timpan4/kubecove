...
Resource Browser and GitOps provide `Refresh (clear cache)` for the current view. It clears the relevant backend cache and reloads active reads, including open details, without resetting filters or selection. Each resource watch reports its own health. While any watch is disconnected, KubeCove retries startup and reloads the view at the existing 15-second Argo inspection cadence; established watches reconnect in the backend.
